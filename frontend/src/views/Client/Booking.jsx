...
const formatPrice = (v) =>
  v ? Number(String(v).replace(",", ".")).toLocaleString("pt-BR") : "";
...
export default function Booking({ onBack }) {
  ...
  const [waitlistConfirmationData, setWaitlistConfirmationData] =
    useState(null);
  ...
  useEffect(() => {
    ...
    Promise.all([fetchServices(), fetchBarbers()])
      ...
      .catch(
        () =>
          isMounted && setError("Não foi possível carregar os dados iniciais."),
      )
  ...
  const visibleSlotSet = useMemo(() => {
    ...
    return new Set(
      timeSlots.filter(
        (t) =>
          !isToday ||
          parseInt(t.split(":")[0]) * 60 + parseInt(t.split(":")[1]) >= nowMins,
      ),
    );
  }, [selectedDate, today]);

  const availableSet = useMemo(
    () => new Set(availableTimes.filter((t) => visibleSlotSet.has(t))),
  ...
  const timeGroups = [
    {
      title: "MANHÃ",
      slots: [...visibleSlotSet].filter((t) => parseInt(t) < 12),
    },
    {
      title: "TARDE",
      slots: [...visibleSlotSet].filter((t) => parseInt(t) >= 12),
  ...
  useEffect(() => {
    ...
    Promise.all(
      others.map((b) =>
        fetchAvailableTimes(b.id, formatDateISO(selectedDate)).catch(
          () => ({}),
        ),
      ),
    )
      .then((results) => {
        if (!isMounted) return;
        const alts = others.filter((_, i) =>
          (results[i]?.availableTimes || []).some((t) => visibleSlotSet.has(t)),
        );
        setAlternativeBarbers(alts);
      })
      .finally(
        () => isMounted && setLoading((l) => ({ ...l, alternatives: false })),
      );
  ...
  const fetchTimes = async (bId) => {
    ...
    try {
      const { availableTimes: times } = await fetchAvailableTimes(
        bId,
              ...
              {step === 0 && (
                <section className="booking-section">
                  <HeaderTitle
                    ...
                    sub="Selecione o que deseja realizar"
                  />
                  <div className="booking-divider" />

                  {loading.data ? (
                    <div className="booking-loader">Carregando...</div>
                  ) : (
                    <div className="service-grid">
                      {services.map((s, i) => (
                        <button
                          key={s.id}
                          className={`service-card ${selectedServiceId === String(s.id) ? "is-selected" : ""}`}
                          onClick={() => setSelectedServiceId(String(s.id))}
                        >
                          <div
                            className={`service-media service-media--${getServiceVariant(s.name)}`}
                            style={{ backgroundImage: `url(${avatarImage})` }}
                          >
                            {selectedServiceId === String(s.id) && (
                              <span className="service-avatar">
                                <img src={avatarImage} alt="Sel" />
                              </span>
                            )}
                          </div>
                ...
                <section className="booking-section">
                  ...
                  <div className="barber-list">
                    {barbers.map((b) => (
                      <button
                        key={b.id}
                        className={`barber-card ${selectedBarberId === String(b.id) ? "is-selected" : ""}`}
                        onClick={() => setSelectedBarberId(String(b.id))}
                      >
                        <div className="barber-left">
                          <span className="barber-icon" />
                          <span className="barber-name">{b.name}</span>
                        </div>
                        <div className="barber-right">
                          <span
                            className={`barber-check ${selectedBarberId === String(b.id) ? "is-selected" : ""}`}
                          />
              ...
              {step === 2 && (
                <section className="booking-section">
                  <HeaderTitle
                    ...
                    sub="Selecione o dia disponível"
                  />
                  <div className="booking-divider" />
                    ...
                    <div className="calendar-grid">
                      {calendarDays.map((d, i) =>
                        d ? (
                          <button
                            key={d.toISOString()}
                            className={`calendar-day ${d.getTime() === selectedDate.getTime() ? "is-selected" : d < today ? "is-unavailable" : "is-available"}`}
                  ...
                  <button
                    className="booking-cta"
                    onClick={() => fetchTimes(selectedBarberId)}
                    disabled={loading.times}
                  >
                    {loading.times ? "CARREGANDO..." : "VER HORÁRIOS"}
                  </button>
                </section>
              )}
                  ...
                  {loading.times ? (
                    ...
                    <div className="no-availability">
                      ...
                      {loading.alternatives ? (
                        <div className="booking-loader">Buscando...</div>
                      ) : (
                        <div className="barber-list">
                          {alternativeBarbers.map((b) => (
                            <button
                              key={b.id}
                              className={`barber-card ${alternativeBarberId === String(b.id) ? "is-selected" : ""}`}
                              onClick={() =>
                                setAlternativeBarberId(String(b.id))
                              }
                            >
                              <div className="barber-left">
                                <span className="barber-name">{b.name}</span>
                              ...
                              <div className="barber-right">
                                <span
                                  className={`barber-check ${alternativeBarberId === String(b.id) ? "is-selected" : ""}`}
                                />
                              </div>
                            </button>
                          ))}
                      ...
                      <div className="no-availability-actions">
                        ...
                        <button
                          className="booking-cta"
                          onClick={() => fetchTimes(alternativeBarberId)}
                    ...
                    <>
                      ...
                      >
                        {formError && (
                          <div className="booking-error">{formError}</div>
                        )}
                        ...
                        {selectedUnavailableTime ? (
                          <WaitlistEntryPanel
                            ...
                            onSubmit={async ({ barberId, date, time }) => {
                              ...
                              if (alreadyInQueue)
                                throw new Error(
                                  "Você já está na lista de espera para este período.",
                                );
                              ...
                              try {
                                ...
                                const raw =
                                  localStorage.getItem(waitlistCacheKey);
                                const list = raw ? JSON.parse(raw) : [];
